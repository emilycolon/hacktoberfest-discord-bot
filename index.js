...
client.on('message', async msg => {
  ...
  if (command === 'ping') {
    ...
    m.edit(
      `Ponggers! Client ping: \`${m.createdTimestamp -
        msg.createdTimestamp}\`ms. Heartbeat ping: \`${client.ping}\`ms`
    );
  } else if (command === 'hello') {
    await msg.reply(`Hello ${msg.author.username}`);
  } else if (command === 'steak') {
    let attach = new Attachment(
      'https://media.giphy.com/media/9UyZI216ic5vG/giphy.gif'
    );
    msg.channel.send(attach);
  } else if (command === 'halloween') {
    let attach = new Attachment(
      'https://media.giphy.com/media/3oEduI34xe0BG7XbKo/giphy.gif'
    );
    msg.channel.send(attach);
  } else if (command === 'lul') {
    const attachment = new Attachment(
      'https://ubisafe.org/images/lul-transparent-twitch-1.png'
    );
    msg.channel.send(attachment);
  } else if (command === 'status') {
    const m = await msg.channel.send(
      'Querying GitHub for pull request count during Hacktoberfest...'
    );
    ...
    https.get(
      apiUrl,
      {
        headers: {
          'User-Agent':
            'Hacktoberfest bot <https://github.com/RuyiLi/hacktoberfest-discord-bot>'
        }
      },
      resp => {
        let data = '';
        ...
        resp.on('end', () => {
          ...
          if (result.errors) {
            ...
            m.edit(
              `${username} made ${
                result.total_count
              }/5 pull requests during Hacktoberfest 2018`
            );
          }
        });
      }
